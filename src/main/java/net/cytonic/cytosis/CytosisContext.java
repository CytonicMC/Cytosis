...
public class CytosisContext {
    ...
    public <T> T getComponent(Class<T> clazz, boolean createIfMissing) {
        if (createIfMissing && !components.containsKey(clazz)) registerComponent(clazz);
    ...
    public <T> T registerComponent(T component) {
        ...
        if (component instanceof Bootstrappable bootstrappableComponent) bootstrappableComponent.init();
