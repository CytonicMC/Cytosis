...
import net.cytonic.minestomInventoryFramework.context.OpenContext;
import net.cytonic.minestomInventoryFramework.context.RenderContext;
...
public class SnooperMenu extends View {
    ...
    private final State<Byte> permissionState = lazyState(context -> Objects.requireNonNull(Cytosis.getSnooperManager().getChannel(Key.key(idState.get(context)))).recipients());
    private final MutableState<DateRange> dateState = initialState("date");
    private final State<Pagination> paginationState = buildLazyPaginationState(this::fetchData).layoutTarget('1').itemFactory((builder, snoop) -> {
        ...
        context.layoutSlot('>').onRender(slotRenderContext -> {
            if (paginationState.get(slotRenderContext).canAdvance()) {
                ItemStack itemStack = ItemStack.builder(Material.ARROW)
                        .set(DataComponents.ITEM_NAME, Msg.green("Next (%d)", paginationState.get(slotRenderContext).nextPage()))
                        .hideExtraTooltip().build();
                slotRenderContext.setItem(itemStack);
            } else {
                slotRenderContext.setItem(MenuUtils.BORDER);
            }
        }).onClick(slotClickContext -> paginationState.get(slotClickContext).advance()).updateOnStateChange(paginationState);

        context.layoutSlot('<').onRender(slotRenderContext -> {
            if (paginationState.get(slotRenderContext).canBack()) {
                ItemStack itemStack = ItemStack.builder(Material.ARROW)
                        .set(DataComponents.ITEM_NAME, Msg.green("Previous (%d)", paginationState.get(slotRenderContext).lastPage()))
                        .hideExtraTooltip().build();
                slotRenderContext.setItem(itemStack);
            } else {
                slotRenderContext.setItem(MenuUtils.BORDER);
            }
        }).onClick(slotClickContext -> paginationState.get(slotClickContext).back()).updateOnStateChange(paginationState);

        context.layoutSlot('X', ItemStack.builder(Material.BARRIER).customName(Msg.mm("Close")).build()).onClick(SlotClickContext::closeForPlayer);
        context.layoutSlot('#', MenuUtils.BORDER);

        context.layoutSlot('o').onRender(slotRenderContext -> {
            ItemStack itemStack = ItemStack.builder(Material.EYE_ARMOR_TRIM_SMITHING_TEMPLATE)
            ...
            slotRenderContext.setItem(itemStack);
        }).onClick(slotClickContext -> slotClickContext.openForPlayer(SnooperMenu.class, ImmutableMap.of(
    ...
    private List<QueriedSnoop> fetchData(IFContext context) {
        return Cytosis.getSnooperManager().getPersistenceManager()
                .query(
                        idState.get(context),
    ...
    public void onOpen(@NotNull OpenContext context) {
        context.getPlayer().sendMessage(Msg.darkGreySplash("LOADING!", "We're crunching the data for you, just a moment!"));
    }
}
