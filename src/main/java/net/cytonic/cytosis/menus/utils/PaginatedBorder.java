...
public class PaginatedBorder<T> extends ViewComponent {
    ...
    public void onRebuild(@NotNull PlayerView view, @NotNull Player player) {
        if (!pagination.hasNextPage()) {
            ViewItem.bySlot(view, pattern.getSlot('>')).applyPrebuilt(MenuUtils.BORDER);
        } else {
            ViewItem.bySlot(view, pattern.getSlot('>')).material(Material.ARROW)
                .name(Msg.mm("<green>Next (" + pagination.getNextPage() + ")")).hideTooltip(true)
        ...
        if (!pagination.hasPreviousPage()) {
            ...
            ViewItem.bySlot(view, pattern.getSlot('<')).material(Material.ARROW).hideTooltip(true)
    ...
    public void onOpen(@NotNull PlayerView view, @NotNull Player player) {
        try {
            ViewItem.bySlot(view, pattern.getSlot('X')).material(Material.BARRIER).name("Close")
