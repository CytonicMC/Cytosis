...
public class BezierCurveEffect extends KeyframedEffect {
     ...
     * @param duration         The number of ticks to play this keyframed effect over. Must be greater than the revolution
     * @param precision        the precision of this curve. The higher the number, the more accurate the curve segments are.
     *                         It should be noted that a very high precision will cause performance issues, a range
     ...
     * @param strat            The bridging strategy used to connect the points of the curve. For more info, see {@link BridgingStrategy}.
    ...
    public BezierCurveEffect(int resolution, int duration, EasingFunction type, int precision, BridgingStrategy strat, ParticleSupplier particleSupplier, Pos... points) {
        if (points.length < 2) throw new IllegalArgumentException("At least 2 points must be supplied.");
    ...
    private List<Pos> sample(int resolution, List<Pos> ctrl) {
        if (resolution < 2) throw new IllegalArgumentException("resolution must be greater than 2.");
    ...
    private List<Pos> sampleArc(int resolution, int precision, List<Pos> ctrl) {
        ...
        for (int i = 1; i < precision; i++)
            dist[i] = dist[i - 1] +
                    dense.get(i).sub(dense.get(i - 1)).asVec().length();
    ...
    List<Keyframe<?>> computeCurve(ParticleSupplier supplier, int resolution, int precision, List<Pos> ctrl, BridgingStrategy bridge) {
        List<Pos> points = sampleArc(resolution, precision, ctrl);
        List<Keyframe<?>> keyframes = new ArrayList<>();
        for (int i = 0; i < points.size(); i++) {
            if (i == 0 || i == points.size() - 1) continue; // don't compute bridge
