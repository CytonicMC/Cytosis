...
public class MsgCommand extends CytosisCommand {
    ...
    public MsgCommand() {
        ...
        playerArg.setSuggestionCallback((sender, ctx, suggestion) -> network.getOnlinePlayers().getValues()
            .forEach(player -> suggestion.addEntry(new SuggestionEntry(player))));
        setDefaultExecutor((sender, ctx) -> sender.sendMessage(Msg.mm("<RED>Usage: /msg <player> [message]")));
    ...
    private void sendMessage(CytosisPlayer actor, UUID recipient, String message, CytonicNetwork network) {
        ...
        Cytosis.CONTEXT.getComponent(MysqlDatabase.class).addPlayerMessage(actor.getUuid(), recipient, message);
        Cytosis.CONTEXT.getComponent(NatsManager.class).sendChatMessage(
            new ChatMessage(List.of(recipient), ChatChannel.PRIVATE_MESSAGE,
        ...
        actor.sendMessage(Msg.mm("<dark_aqua>To <reset>").append(targetRank.getPrefix().append(
                Msg.mm(Cytosis.CONTEXT.getComponent(CytonicNetwork.class).getLifetimeFlattened().getByKey(recipient))))
