        ...
        addSyntax((sender, context) -> {
            ...
            pm.openParty(player.getUuid(), state)
                ...
                .thenAccept(p -> {
                    if (p.success()) return;
                    switch (p.message()) {
                        case "INTERNAL_ERROR" -> player.error("An error occurred whilst opening the party.");
                        case "NOT_IN_PARTY", "ERR_INVALID_PARTY" -> player.whoops("You are not in a party.");
                        case "ERR_NO_PERMISSION" -> player.whoops("You must be the party leader to open the party.");
                        case "ERR_ALREADY_STATE" ->
                            player.whoops("The party has already been %s.", state ? "opened" : "closed");
