    ...
    public BanCommand() {
        ...
        addSyntax((sender, context) -> {
            if (sender instanceof CytosisPlayer actor) {
                ...
                databaseManager.getMysqlDatabase().isBanned(uuid).whenComplete((banned, throwable1) -> {
                    if (throwable1 != null) {
                    ...
                    databaseManager.getMysqlDatabase().getPlayerRank(uuid).whenComplete((playerRank, throwable2) -> {
                        ...
                        databaseManager.getMysqlDatabase().banPlayer(uuid, reason, dur).whenComplete((ignored, throwable3) -> {
                            if (throwable3 != null) {
                                actor.sendMessage(Msg.mm("<red>An error occured whilst banning " + player + "!"));
                                return;
                            }

                            Cytosis.CONTEXT.getComponent(NatsManager.class).kickPlayer(uuid, KickReason.BANNED, Msg.formatBanMessage(new BanData(reason, dur, true)));
