...
public class NativeImageFeature implements Feature {
    ...
    public void beforeAnalysis(BeforeAnalysisAccess access) {
        ...
        try (ScanResult scanResult = new ClassGraph()
                ...
                .ignoreMethodVisibility()
                .ignoreFieldVisibility()
                .acceptPackages("io.minio")
                .scan()) {
            scanResult.getSubclasses("io.minio.BaseArgs").forEach(ci -> {
                Class<?> clazz = access.findClassByName(ci.getName());

                if (clazz == null) {
                    IO.println("Could not resolve " + ci.getName());
                    return;
                }

                IO.println("Registering MinIO reflection metadata for " + clazz.getName());

                RuntimeReflection.register(clazz);
                RuntimeReflection.registerClassLookup(clazz.getName());

                // Explicitly preserve the no-arg constructor.
                try {
                    Constructor<?> ctor = clazz.getDeclaredConstructor();
                    RuntimeReflection.register(ctor);
                } catch (NoSuchMethodException ignored) {
                    System.err.println("No no-arg constructor on " + clazz.getName());
                }

                // Optional: preserve every constructor as well.
